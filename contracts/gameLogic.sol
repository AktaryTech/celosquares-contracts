...
contract Game is Context, Ownable {
    ...
    constructor(string memory firstTeam, string memory secondTeam, uint256 bet) {
        _owner = _msgSender();
        teamOne = firstTeam;
        teamTwo = secondTeam;
        betAmount = bet; 
    ...
    function placeBet(uint256 x, uint256 y) public {
        require(Square[x][y].hasBet == false, "This square already has a bet placed");
        Square storage mySquare = Square[x][y];
